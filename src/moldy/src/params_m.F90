...
module params_m
  ...
  type simparameters
     ...
     real(kind_wp) :: tempsp !< Difference in Temperatures for Thermal gradient
     real(kind_wp) :: rqke   !< Required kinetic energy
     !!currently calculated from temprq. Convert at i/o time?
...
contains
  ...
  subroutine readparam_keyvaluepair(iunit,ierror)
    ...
    integer, parameter :: numkeys=44  !< increase numkeys when adding keywords
    ...
    case(44) !'tempsp'
!       read(inputstring(eqindex+1:),*) tempsp
!       write(0,*) key(inum)(:keylength(inum))//" = "//tempsp
