...
200 format(' QUENCH, FINDING MINIMUM ENERGY CONFIGURATION OF', &
    ...
    do i=1,simparam%nm
       ...
       y3(i)=0.0
       z3(i)=0.0
       if(simparam%ivol.ne.3) x0(i)=x0(i)-aint(2.0*x0(i))
       if(simparam%ivol.ne.3) y0(i)=y0(i)-aint(2.0*y0(i))
       if(simparam%ivol.le.1.or.simparam%ivol.eq.4) z0(i)=z0(i)-aint(2.0*z0(i))
    end do
...
!    simparam=get_params()
    ...
    nm3 = 3*simparam%nm
    if(simparam%ivol.lt.1.or.simparam%ivol.ge.4) then
       do i=1,nmat
          do j=1,nmat
             nm3=nm3+1
