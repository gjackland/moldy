...
## Run steps WEAK SCALING version 1.0	##
## by Martin Uhrin				##
...
readonly input_file="system.in"	 		# Input file to use
...
## Run settings ##
readonly num_threads=(14 15)	# OMP_NUM_THREADS numbers to use
readonly num_repeats=3		# Number of times to repeat runs for each set of variables
declare -ir num_repeats_x=24	# Number of repeats of basis in x dirn
declare -ir num_repeats_y=24	# Number of repeats of basis in y dirn
declare -ir num_repeats_z=16	# Initial number of repeats in z dirn (this will be multiplied by np)
...
do
	# Edit the system input file the have the specified number of repeats
	sed -r "2 s/[[:digit:]]*[[:blank:]]+[[:digit:]]*[[:blank:]]+[[:digit:]]*/$num_repeats_x $num_repeats_y $[$i * $num_repeats_z]/" \
	$input_file > tmpfile && mv tmpfile $input_file

	# Let's make sure the number of atoms in system.in and params.in agree
	java -classpath "$tools_reldir" FixParams $input_file params.in
