...
program moldyv2
  ...
  integer :: i,j,iter                       !< local loop variables
  ...
  call set_params(simparam)
     ...
     !Set deltat^2 / 2m for respective masses     
     del2(:)=simparam%deltat*simparam%deltat/(2.0*atomic_mass_reference(atomic_number(ispec(1:simparam%nspec))))
     write(*,*) "reference del2 ", del2
     del2(:)=simparam%deltat*simparam%deltat/(2.0*atomic_mass(ispec(1:simparam%nspec)))
     write(*,*) "read/used del2 ", del2     
  ...
  simulationloops: do iter=1,simparam%nloops
    ...
    !!  else
        ...
        do i=1,simparam%nm
           ...
           !!    std term       p-r term
           x2(i)=del2(j)*fx(i)-0.5*a1
           y2(i)=del2(j)*fy(i)-0.5*a2
           z2(i)=del2(j)*fz(i)-0.5*a3
        end do
...
contains
  ...
  subroutine read_checkpoint_file
    ...
    write(unit_stdout,'("Reading checkpoint file...",//)')
